...
import (
	...
	"github.com/aflockgate/aflockgate/internal/attest"
	"github.com/aflockgate/aflockgate/internal/gate"
	"github.com/aflockgate/aflockgate/internal/guac"
	"github.com/aflockgate/aflockgate/internal/output"
	"github.com/aflockgate/aflockgate/internal/policy"
	"github.com/aflockgate/aflockgate/internal/scanner"
...
func main() {
	...
	prNumber := 0
	fmt.Sscanf(os.Getenv("AFLOCKGATE_PR_NUMBER"), "%d", &prNumber)
	...
	policyPath := envOrDefault("AFLOCKGATE_POLICY_PATH", ".aflock.json")
	repoRoot := envOrDefault("GITHUB_WORKSPACE", ".")
	outputDir := envOrDefault("AFLOCKGATE_OUTPUT_DIR", filepath.Join(repoRoot, "aflockgate-artifacts"))
	signingKey := os.Getenv("AFLOCK_SIGNING_KEY_B64")
	...
	if pol.IsGateMode() && gateResult.Decision == gate.DecisionFail {
		log.Fatalf("AflockGate: Quality gate FAILED — %d blocking finding(s)", len(gateResult.Reasons))
	}
}
